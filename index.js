...
function adjustHealth(entity, by) {
    ...
    let change = newHealth - entity.hp;
    entity.hp = newHealth;
    return change;
}
...
async function handleConsumable(entity) {
    switch (entity.consumable.type) {
        ...
        case 'confusion': {
            ...
            let position = await Layer.chooseEnemy.waitForAnswer();
        ...
        case 'fireball': {
            print `Select a target location.`;
            let position = await Layer.choosePosition.waitForAnswer({radius: entity.consumable.radius});
            if (!position) return false; // cancelled
