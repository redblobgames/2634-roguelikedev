...
test('Table findExactlyOne and findAny handle strict cardinality', () => {
    let prototypes = { tile: {} };
    let table = new Table('test', ['position'], prototypes);
    let x = table.create('tile', { position: [1, 1] });

    let found = table.findExactlyOne({ position: [1, 1] });
    assert.strictEqual(found.id, 1);
    let y = table.create('tile', { position: [2, 1] });
    assert.throws(() => table.findExactlyOne({ type: 'tile' }));
