...
export class Table {
    ...
    /** Convenience: make sure exactly one row matched */
    findExactlyOne(query) {
        let results = this.findAll(query);
        if (results.length !== 1) throw `findExactlyOne() matched ${results.length} rows`;
        return results[0];
    }

    /** Convenience: make sure at most one row matched */
    findAny(query) {
        let results = this.findAll(query);
    ...
    /** Convenience: return first of the resulting rows, as it doesn't matter which matched */
    findFirst(query) {
        let results = this.findAll(query);
        if (results.length === 0) return null;
        return results[0];
